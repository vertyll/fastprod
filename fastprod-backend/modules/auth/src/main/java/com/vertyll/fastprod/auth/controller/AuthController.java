...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<Void>> logoutAll(HttpServletRequest request, HttpServletResponse response) {
        ...
        return ApiResponse.buildResponse(null, LOGGED_OUT_FROM_ALL_SESSIONS_SUCCESSFULLY, HttpStatus.OK);
    }

    @GetMapping("/sessions")
    @PreAuthorize(IS_AUTHENTICATED)
    @Operation(summary = "Get all active sessions for the current user")
    public ResponseEntity<ApiResponse<List<SessionResponseDto>>> getSessions(@AuthenticationPrincipal String email) {
        List<SessionResponseDto> sessions = authService.getUserActiveSessions(email);
        return ApiResponse.buildResponse(sessions, ACTIVE_SESSIONS_RETRIEVED_SUCCESSFULLY, HttpStatus.OK);
    }
