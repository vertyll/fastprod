...
public abstract class BaseHttpService {
    ...
    protected BaseHttpService(
            String backendUrl, ObjectMapper objectMapper, AuthTokenProvider authTokenProvider) {
    ...
    protected <T> ApiResponse<T> get(String endpoint, Class<T> responseType) throws Exception {
        HttpRequest.Builder requestBuilder =
                HttpRequest.newBuilder().uri(URI.create(backendUrl + endpoint)).GET();
    ...
    protected <T> ApiResponse<T> delete(String endpoint, Class<T> responseType) throws Exception {
        HttpRequest.Builder requestBuilder =
                HttpRequest.newBuilder().uri(URI.create(backendUrl + endpoint)).DELETE();
        ...
        if (response.statusCode() >= 200 && response.statusCode() < 300) {
            ...
            try {
                ...
                throw new ApiException(
                        "Error occurred during server communication", response.statusCode());
            }
        }
    }

    private <T> PageResponse<T> handlePaginatedResponse(
            HttpResponse<String> response, Class<T> responseType) throws Exception {
        if (response.statusCode() >= 200 && response.statusCode() < 300) {
                    ...
                    objectMapper.readValue(
                            ...
                            objectMapper
                                    ...
                                    .constructParametricType(
                                            PaginatedApiResponse.class, responseType));
